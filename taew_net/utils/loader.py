...
def load_edin_data(_path, coords, joints, num_labels, frame_drop=1):
    ...
    labels /= (num_annotators * 2.) # (1835, 10) 10 annotators
    ...
    num_samples = labels.shape[0] # 1835
    balanced_emo_idx = balance_edin_emotion_labels(labels) # (3508,)
    data = data[balanced_emo_idx, :, :] # (3508, 48, 63)
    poses = poses[balanced_emo_idx, :] # (3508, 63)
    rotations = rotations[balanced_emo_idx, :, :] # (3508, 48, 84)
    translations = translations[balanced_emo_idx, :, :] # (3508, 48, 63)
    affective_features = affective_features[balanced_emo_idx, :, :] # (3508, 48, 18)
    num_frames = num_frames[balanced_emo_idx] # (3508,)
    labels = labels[balanced_emo_idx, :] # (1835,)
    labels[:-num_samples, :] = 0 # (3508, 10)
    label_weights = np.zeros(4) #
    for c in range(len(label_weights)):
        label_weights[c] = len(labels) / len(np.where(labels[:, c] > 0.25)[0])
    num_unlabeled = len(balanced_emo_idx) - num_samples # 1673
    start_idx = num_samples + num_unlabeled # 3508
    data = data[-start_idx:, :, :] #
...
def      balance_edin_emotion_labels(labels):
    labels_emo = labels[:, :4] # (1835, 4)
    labels_oh = np.zeros_like(labels_emo) # (1835, 4)
    labels_oh[np.arange(len(labels_emo)), labels_emo.argmax(1)] = 1 # (1835, 4)
    c_idx = []
