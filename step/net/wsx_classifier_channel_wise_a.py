...
class unit_gcn_channel_wise_a(nn.Module):
    def __init__(self, in_channels, out_channels, A):
        super(unit_gcn_channel_wise_a, self).__init__()
        self.out_c = out_channels
        self.in_c = in_channels
        self.num_subset = A.shape[0]
        self.num_point = A.shape[1]

        CPA = np.stack([np.eye(self.num_point)] * self.out_c, axis=0)

        self.PA = nn.Parameter(torch.from_numpy(CPA.astype(np.float32)), requires_grad=True)

        self.point_conv = nn.Conv2d(self.in_c, self.out_c, kernel_size=1)
    ...
    def forward(self, x):
        ...
        A = self.PA
        A = self.L2_norm(A)
        A = A.unsqueeze(0).repeat(N, 1, 1, 1)
        x = self.point_conv(x)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, temporal_receptive_field=1, residual=True, adaptive=True):
        ...
        self.gcn1 = unit_gcn_channel_wise_a(in_channels, out_channels,  A)
