...
weight_path = os.path.join(base_path, '../work_dir')
...
parser.add_argument('--work-dir', type=str, default=model_path, metavar='WD',
                    help='path to save')
parser.add_argument('--weight-dir', type=str, default=weight_path, metavar='WD',
                    help='weight to load')
...
data, labels, [data_train, data_test, labels_train, labels_test] =\
        loader.load_edin_data(
            '/wsx/take_an_emotion_walk/taew_net/../datasets/data_edin_locomotion_pose_diff_aff_drop_5.npz'.format(args.frame_drop),
...
if args.train:
    ...
    pr = processor.Processor(args, data_loader_train_test, coords, num_classes, device=device, verbose=True)
    pr.train()
else:
    pr = processor.Processor(args, None, coords, num_classes, device=device, verbose=False)
